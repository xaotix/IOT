    ...
    public class Utilz
    {
        ...
        public static int GetId(string ma)
        {
            if(ma.Length>=6)
            {
                ...
                rg.Filtros.Add(new Celula("user", ma));
            ...
            if (ma.Length >= 6)
            {
                ...
                if (users.Linhas.Count > 0)
                {
                    ...
                    if(user.Get("s").ToString() == "")
                    {
                        ...
                        var senha = Utilz.Criptografia.Criptografar(s, Vars.Chave_Criptografia);

                        user.Set("s", senha);
                        Registro pp = new Registro();
                        pp.Tabela = Vars.TabelaUsers;
                        pp.Banco = Vars.BancoUsers;
                        pp.Filtros.Add(new Celula("user", ma));
        ...
        public static Tabela Logar(Tabela retorno)
        {
            ...
            try
            {
                ...
                if (retorno.user == null | retorno.user == "" | retorno.s == "" | retorno.s == null)
                {
                    retorno.Status = "Faltam dados de login.";
                    return retorno;
                }
                else if(!regexItem.IsMatch(retorno.user))
                {
                    retorno.Status = "USER não pode conter caracteres especiais.";
                    return retorno;
                }
                else if (retorno.user.Length < 6)
                {
                    retorno.Status = "USER deve conter pelo menos 6 caracteres.";
                    return retorno;
                }
                ...
                if (Banco.Tipo == Tipo_Conexao.SemActiveDirectory)
                {
                    ...
                    var s = GetUser(retorno.user, retorno.s);
                ...
                {
                    System.DirectoryServices.DirectoryEntry directoryEntry = new System.DirectoryServices.DirectoryEntry(Vars.LDAP, retorno.user, retorno.s);
                    DirectorySearcher busca = new DirectorySearcher(directoryEntry);
                    busca.PageSize = 1000;
                    busca.SearchScope = SearchScope.Subtree;
                    busca.Filter = "(&(samAccountType=805306368)(sAMAccountName=" + retorno.user + "))";
                    busca.PropertiesToLoad.Add("givenName");   // nome
                    busca.PropertiesToLoad.Add("sn");          // sobrenome
                    busca.PropertiesToLoad.Add("mail");        // email

                    SearchResult resultados = null;

                    resultados = busca.FindOne();

                    if (resultados != null)
                    {
                        retorno.id_user = GetId(retorno.user);
                        ...
                        if (retorno.id_user < 0)
                        {
                            ...
                            rg.Valores.Add(new Celula("user", retorno.user));
